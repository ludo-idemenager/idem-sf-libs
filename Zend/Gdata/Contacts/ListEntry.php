...
require_once 'Zend/Gdata/Contacts/Extension/GroupMembershipInfo.php';
require_once 'Zend/Gdata/Contacts/Extension/UserDefinedField.php';
...
class Zend_Gdata_Contacts_ListEntry extends Zend_Gdata_Entry
{
	...
	public function getDOM($doc = null, $majorVersion = 1, $minorVersion = null) {
		...
		if ($this->_groups != null) {
			foreach ($this->_groups as $group) {
				$element->appendChild($group->getDOM($element->ownerDocument));
			}
		}
		if ($this->_userDefinedFields != null) {
			foreach ($this->_userDefinedFields as $userDefinedField) {
				$element->appendChild($userDefinedField->getDOM($element->ownerDocument));
			}
		}
		return $element;
	}
	...
	protected function takeChildFromDOM($child) {
		...
		case $gdNamespacePrefix . 'phoneNumber':
            ...
            break;
        case $this->lookupNamespace('gContact') . ':' . 'groupMembershipInfo':
            $groupInfo = new Zend_Gdata_Contacts_Extension_GroupMembershipInfo();
            $groupInfo->transferFromDOM($child);
            $this->_groups[] = $groupInfo;
            break;
        case $this->lookupNamespace('gContact') . ':' . 'userDefinedField':
            $userDefinedField = new Zend_Gdata_Contacts_Extension_UserDefinedField();
            $userDefinedField->transferFromDOM($child);
            $this->_userDefinedFields[] = $userDefinedField;
            break;
        default:
     ...
     *
     * @todo return primary first, if any
     * @return array An array of Zend_Gdata_Contacts_Extension_GroupMembershipInfo objects
     */
    public function getGroups() {
		return $this->_groups;
	}
    /**
     * @param array $values Array of Zend_Gdata_Contacts_Extension_GroupMembershipInfo items
     * @return Zend_Gdata_Extension_ListEntry or else FALSE on error
     */
	...
	public function setUserDefinedFields($values) {
		$this->_userDefinedFields = $values;
		return $this;
	}
